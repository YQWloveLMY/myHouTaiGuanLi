    ...
    return new Promise((resolve, reject) => {
      getInfo(state.token)
        .then((response) => {
          ...
          //vuex存储用户全部的信息,data是返回的数据
          commit("SET_USERINFO", data);
          //   计算出来应该要展示的路由

          //   console.log(data.routes, "这是服务器返回的信息"); //有品牌管理
          //   console.log(asyncRoutes, "这是有的异步路由"); //有品牌管理
          //   let a = computedAsyncRoutes(cloneDeep(asyncRoutes), data.routes); //对比函数
          //   console.log(a, "这是对比结果"); //对比结果，品牌管理没了
          commit(
...
const getDefaultState = () => {
  return {
    ...
    //按钮权限的信息
    buttons: [],
    //对比之后【项目中已有的异步路由，与服务器返回的标记信息进行对比最终需要展示的理由】 ---要展示的异步路由
    resultAsyncRoutes: [],
    //用户最终需要展示全部路由
    resultAllRputes: [],
  };
...
const computedAsyncRoutes = (asyncRoutes, routes) => {
  ...
  return asyncRoutes.filter((item) => {
    ...
    //这里注意，TradeMark在权限中是小写，但是在路由中是大写，由于includes和indexOf都是匹配大小写的，所以会匹配失败
    // console.log(item.name, routes.includes(item.name) != false);
    if (routes.includes(item.name) != false) {
